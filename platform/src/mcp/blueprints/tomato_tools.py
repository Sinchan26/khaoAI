...
@bp.route(route="tomato/search", methods=["POST", "GET"], auth_level=func.AuthLevel.ANONYMOUS)
async def tomato_search(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("Executing Tomato search tool.")
    try:
        if req.method == "POST":
        ...
        logging.error(f"Error in tomato_search: {str(e)}")
        return func.HttpResponse(
            body=json.dumps({"error": str(e), "platform": "Tomato", "results": []}),
            status_code=500,
            mimetype="application/json"
        )

@bp.route(route="tomato/restaurant/{restaurant_id}", methods=["GET"], auth_level=func.AuthLevel.ANONYMOUS)
async def tomato_restaurant(req: func.HttpRequest) -> func.HttpResponse:
    logging.info("Executing Tomato restaurant detail tool.")
    try:
        ...
        logging.error(f"Error in tomato_restaurant: {str(e)}")
        return func.HttpResponse(
